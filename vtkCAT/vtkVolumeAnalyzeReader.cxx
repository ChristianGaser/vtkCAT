...
#include "vtkVolumeAnalyzeReader.h"
#include "dbh.h"

vtkVolumeAnalyzeReader* vtkVolumeAnalyzeReader::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkVolumeAnalyzeReader");
  if(ret)
    {
    return (vtkVolumeAnalyzeReader*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkVolumeAnalyzeReader;
}

// Construct object 
vtkVolumeAnalyzeReader::vtkVolumeAnalyzeReader()
{
...
void vtkVolumeAnalyzeReader::PrintSelf(ostream& os, vtkIndent indent)
{
   // the reader ivar's source will be this reader. we must do this to
   // prevent infinite printing
   //if (!recursing) {
      vtkVolumeRawReader::PrintSelf(os,indent);
...
void vtkVolumeAnalyzeReader::ReadHeader()
{
...
void vtkVolumeAnalyzeReader::Execute()
{
  this->ReadHeader();
  if ((haveSwapBytes == 0) ||
      (haveDimensions == 0)) {
