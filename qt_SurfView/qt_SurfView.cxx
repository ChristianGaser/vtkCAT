...
int main( int argc, char **argv )
{
  ...
  QMenu* menu = window->menuBar()->addMenu(QObject::tr("&File"));

//  menu->addAction("&Open", window, SLOT(open()));
//  menu->addAction("&Save", window, SLOT(save()));
    menu->addAction(QObject::tr("&Quit"), app, SLOT(quit()));
//  menu->addAction("&Quit2", window, SLOT(close()));
  ...
  window->setWindowTitle(inputFileName);
