...
void FastLoop::_InitFaceData(void)
{
	...
	for(int n = 0 ; n < nfaces ; n++){
		...
		fdst->x=fsrc->x;fdst->y=fsrc->y;fdst->z=fsrc->z;
		for(int i = 0 ; i < 3 ; i++){
			fdst->v[i]=fsrc->v[i];
			int fn = fsrc->f[i];
...
void FastLoop::_FindFacePath(Loop &loop, int init_fn1,int init_fn2)
{
	...
	// first path
	path[0] = 0; nfaces[0] = 0 ; next_fn = init_fn1;
	...
	path[1] = 0; nfaces[1] = 0; next_fn=init_fn1;
...
int FastLoop::_SimplifyLoop(Loop &loop)
{
	...
	if(npoints==0) 
		return _OrderLoop(loop); ; //no triple face to remove
