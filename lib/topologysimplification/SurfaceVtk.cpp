...
vtkPolyData* Surface::SurfaceToVtk() const
{
	if ((nfaces > 0) && (nvertices > 0)) {
               
       vtkCellArray* polys = vtkCellArray::New();
       int size = polys->EstimateSize(nfaces, 3);
       polys->Allocate(size, 25);
       for (int i = 0; i < nfaces; i++) {
          polys->InsertNextCell(3, faces[i].v);
       }
       
       vtkPoints* pointData = vtkPoints::New();
       for (int j = 0; j < nvertices; j++) {
          pointData->InsertPoint(j, vertices[j].x, vertices[j].y, vertices[j].z);
       }
...
int Surface::VtkToSurface(vtkPolyData* polyData)
{
    double xyz[3];
